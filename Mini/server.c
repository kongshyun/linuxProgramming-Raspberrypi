...
int client_count = 0;
int pipe1[MAX_CLIENTS][2]; // Child -> Parent
int pipe2[MAX_CLIENTS][2]; // Parent -> Child
...
void handle_sigchld(int sig) {
    ...
    // Loop to clean up all terminated children
    while ((pid = waitpid(-1, &status, WNOHANG)) > 0) {
        // Remove the client associated with the terminated child process
        for (int i = 0; i < client_count; i++) {
            if (clients[i].pid == pid) {
                printf("[SERVER] Client %s (PID %d) disconnected.\n", clients[i].username, pid);
                // Shift the remaining clients to fill the gap
                for (int j = i; j < client_count - 1; j++) {
                    clients[j] = clients[j + 1];
                    pipe1[j][0] = pipe1[j + 1][0];
                    pipe1[j][1] = pipe1[j + 1][1];
                    pipe2[j][0] = pipe2[j + 1][0];
                    pipe2[j][1] = pipe2[j + 1][1];
                }
                client_count--;
                break;
            }
...
void handle_pipe_read(int sender_index) {
    Message msg;
    if (read(pipe1[sender_index][0], &msg, sizeof(Message)) > 0) {
...
void handle_child_signal(int sig) {
    ...
    // Find the index of the current child process
    for (int i = 0; i < client_count; i++) {
    ...
    if (client_index != -1) {
        ...
        if (read(pipe2[client_index][0], &msg, sizeof(Message)) > 0) {
